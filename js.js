...
projects.forEach(p => {
  ...
  el.innerHTML = `
    ...
    <div class="project-img slider">
      ...
      <div class="slider-counter">1 / ${p.image.length}</div>
      <div class="slider-dots">
        ${Array.from({length: p.image.length}, (_, i) => 
...
const modalSliderCounter = document.getElementById('modalSliderCounter');
...
function renderModalDots(images, currentIndex) {
  return `
    ${images.map((_, i) => 
...
grid.addEventListener('click', (e) => {
  if(e.target.classList.contains('details-btn')){
    ...
    modalSliderDots.innerHTML = renderModalDots(images, index);
    modalSliderCounter.textContent = `${index + 1} / ${slides.length}`;
